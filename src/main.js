...
async function setupAudio() {
	...
	for (let i = 0; i < 2; i++) {
		...
		for (let j = 0; j < length; j++)
			ch[j] = (Math.random() * 2 - 1) * (1 - j / length) ** 5;
	}
...
function playNote(time, cell) {
	...
	const freq =
		chords[currentChord][cells.indexOf(cell) % 2] *
		(isTick ? 2 ** cell.depth : 0.5);
...
function playClick(time) {
	...
	const safeFreq = Math.min(
		22000,
		2000 + (cells.length / 16) * 4000 + Math.random() * 500,
	);
...
function createCell(container, depth) {
	...
	if (depth < 5 && Math.random() > currentDensity) {
		...
		const ratio = `${Math.floor(Math.random() * 3) + 1}fr ${
